...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  ...
  const sessdata = config.SESSION_ID.replace("Mawrld~", '')
...
async function connectToWA() {
  try {
    console.log("[ ♻ ] MAWRLD MD CONNECTING TO WHATSAPP ⏳️...")
    ...
    conn.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        try {
          console.log('[ 🧬 ] INSTALLING MAWRLD MD PLUGINS')
          ...
          let up = `*Hᴇʟʟᴏ ᴛʜᴇʀᴇ 𝙼𝙰𝚆𝚁𝙻𝙳 𝙼𝙳 ᴄᴏɴɴᴇᴄᴛᴇᴅ! 👋🏻* 
...
> © ᴘᴏᴡᴇʀᴇᴅ ʙʏ Rɪᴅᴢ Cᴏᴅᴇʀ 🇿🇼`;
    conn.sendMessage(conn.user.id, { image: { url: `https://files.catbox.moe/qwpimr.png` }, caption: up })

          const channelJid = "120363404529319592@newsletter"
          try {
